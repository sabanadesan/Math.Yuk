    ...
    class Program
    {
                                             ...
                                             bool enhancedMode = true)
        {
            ...
            while (true)
            {
                ...
                {
                    delta = System.Math.Max(-1, System.Math.Min(++delta, sb.Length - 1));
                ...
                if (key.Key == ConsoleKey.Backspace || key.Key == ConsoleKey.Delete)
                {
                    if (sb.Length > 0)
                    {
                        delta = key.Key == ConsoleKey.Backspace ?
                          System.Math.Max(-1, System.Math.Min(--delta, sb.Length - 2)) : delta;
                        if (delta < sb.Length - 1)
                        {
                            sb.Remove(delta + 1, 1);
                        }
                        SetCursor(prompt, sb.ToString(), System.Math.Max(0, delta + 1));
                    }
                    continue;
                }
                ...
                if (delta < sb.Length)
                {
                    delta = System.Math.Max(0, System.Math.Min(delta, sb.Length - 1));
