...
namespace Math.Lang
{
    public class Variable
    {
        ...
        public bool Exists(Variable indexVar, bool notEmpty = false)
        {
            ...
            if (indexVar.Type == VarType.NUMBER)
            {
                if (indexVar.Value < 0 ||
                    ...
                    indexVar.Value - System.Math.Floor(indexVar.Value) != 0.0)
                {
                    return false;
                }
                if (notEmpty)
                                       ...
                                       int maxCount = -1)
        {
            ...
            int count = maxCount < 0 ? m_tuple.Count : System.Math.Min(maxCount, m_tuple.Count);
            int i = 0;
            if (m_dictionary.Count > 0)
            {
                count = maxCount < 0 ? m_dictionary.Count : System.Math.Min(maxCount, m_dictionary.Count);
        ...
        Variable GetCoreProperty(string propName, ParsingScript script = null)
        {
            ...
            else if (script != null && propName.Equals(Constants.SUBSTRING, StringComparison.OrdinalIgnoreCase))
            {
                ...
                length = System.Math.Min(length, AsString().Length - startFrom);
