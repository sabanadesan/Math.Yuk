...
namespace Math.Lang
{
    ...
    class EFunction : ParserFunction, INumericFunction
    {
        protected override Variable Evaluate(ParsingScript script)
        {
            return new Variable(System.Math.E);
        }
    }
    class Sqrt2Function : ParserFunction, INumericFunction
    {
        protected override Variable Evaluate(ParsingScript script)
        {
            return new Variable(System.Math.Sqrt(2));
        }
    }
    ...
    class Ln2Function : ParserFunction, INumericFunction
    {
        protected override Variable Evaluate(ParsingScript script)
        {
            return new Variable(System.Math.Log(2));
        }
    }
    class Ln10Function : ParserFunction, INumericFunction
    {
        protected override Variable Evaluate(ParsingScript script)
        {
            return new Variable(System.Math.Log(10));
        }
    }
    class Log2EFunction : ParserFunction, INumericFunction
    {
        protected override Variable Evaluate(ParsingScript script)
        {
            return new Variable(System.Math.Log(System.Math.E, 2)) ;
        }
    }
    ...
    class CosFunction : ParserFunction, INumericFunction
    {
        protected override Variable Evaluate(ParsingScript script)
        {
            ...
            Utils.CheckArgs(args.Count, 1, m_name, true);
            Variable arg = args[0];
            arg.Value = System.Math.Cos(arg.Value);
            return arg;
        }
    }
    class TanFunction : ParserFunction, INumericFunction
    {
        protected override Variable Evaluate(ParsingScript script)
        {
            List<Variable> args = script.GetFunctionArgs();
            Utils.CheckArgs(args.Count, 1, m_name, true);
            return new Variable(System.Math.Tan(args[0].Value));
        }
    }
    class SinhFunction : ParserFunction, INumericFunction
    {
        protected override Variable Evaluate(ParsingScript script)
        {
            ...
            Utils.CheckArgs(args.Count, 1, m_name, true);
            Variable arg = args[0];
            arg.Value = System.Math.Sinh(arg.Value);
            return arg;
        }
    }
    class CoshFunction : ParserFunction, INumericFunction
    {
        protected override Variable Evaluate(ParsingScript script)
        {
            ...
            Utils.CheckArgs(args.Count, 1, m_name, true);
            Variable arg = args[0];
            arg.Value = System.Math.Cosh(arg.Value);
            return arg;
        }
    }
    class TanhFunction : ParserFunction, INumericFunction
    {
        protected override Variable Evaluate(ParsingScript script)
        {
            List<Variable> args = script.GetFunctionArgs();
            Utils.CheckArgs(args.Count, 1, m_name, true);
            return new Variable(System.Math.Tanh(args[0].Value));
        }
    }
    class AsinFunction : ParserFunction, INumericFunction
    {
        protected override Variable Evaluate(ParsingScript script)
        {
            ...
            Utils.CheckArgs(args.Count, 1, m_name, true);
            Variable arg = args[0];
            arg.Value = System.Math.Asin(arg.Value);
            return arg;
        }
    }
    class AcosFunction : ParserFunction, INumericFunction
    {
        protected override Variable Evaluate(ParsingScript script)
        {
            ...
            Utils.CheckArgs(args.Count, 1, m_name, true);
            Variable arg = args[0];
            arg.Value = System.Math.Acos(arg.Value);
            return arg;
        }
    }
    class AtanFunction : ParserFunction, INumericFunction
    {
        protected override Variable Evaluate(ParsingScript script)
        {
            ...
            Utils.CheckArgs(args.Count, 1, m_name, true);
            Variable arg = args[0];
            arg.Value = System.Math.Atan(arg.Value);
            return arg;
        }
    }
    class Atan2Function : ParserFunction, INumericFunction
    {
        protected override Variable Evaluate(ParsingScript script)
        {
            List<Variable> args = script.GetFunctionArgs();
            Utils.CheckArgs(args.Count, 2, m_name, true);
            return new Variable(System.Math.Atan2(args[0].Value, args[1].Value));
        }
    }
    class AsinhFunction : ParserFunction, INumericFunction
    {
        protected override Variable Evaluate(ParsingScript script)
        {
            ...
            Utils.CheckArgs(args.Count, 1, m_name, true);
            Variable arg = args[0];
            arg.Value = System.Math.Log(arg.Value + System.Math.Sqrt(arg.Value * arg.Value + 1));
            return arg;
        }
    }
    ...
    class SqrtFunction : ParserFunction, INumericFunction
    {
        protected override Variable Evaluate(ParsingScript script)
        {
            ...
            Utils.CheckArgs(args.Count, 1, m_name, true);
            Variable arg = args[0];
            arg.Value = System.Math.Sqrt(arg.Value);
            return arg;
        }
    }
    class CbrtFunction : ParserFunction, INumericFunction
    {
        protected override Variable Evaluate(ParsingScript script)
        {
            List<Variable> args = script.GetFunctionArgs();
            Utils.CheckArgs(args.Count, 1, m_name, true);
            return new Variable(System.Math.Pow(args[0].Value, 1.0 / 3.0));
        }
    }
    ...
    class AbsFunction : ParserFunction, INumericFunction
    {
        protected override Variable Evaluate(ParsingScript script)
        {
            ...
            Utils.CheckArgs(args.Count, 1, m_name, true);
            Variable arg = args[0];
            arg.Value = System.Math.Abs(arg.Value);
            return arg;
        }
    }
    class SignFunction : ParserFunction, INumericFunction
    {
        protected override Variable Evaluate(ParsingScript script)
        {
            ...
            Utils.CheckArgs(args.Count, 1, m_name, true);
            Variable arg = args[0];
            arg.Value = System.Math.Sign(arg.Value);
            return arg;
        }
    }
    class CeilFunction : ParserFunction, INumericFunction
    {
        protected override Variable Evaluate(ParsingScript script)
        {
            ...
            Utils.CheckArgs(args.Count, 1, m_name, true);
            Variable arg = args[0];
            arg.Value = System.Math.Ceiling(arg.Value);
            return arg;
        }
    }
    ...
    class FloorFunction : ParserFunction, INumericFunction
    {
        protected override Variable Evaluate(ParsingScript script)
        {
            ...
            Utils.CheckArgs(args.Count, 1, m_name, true);
            Variable arg = args[0];
            arg.Value = System.Math.Floor(arg.Value);
            return arg;
        }
    }
    ...
    class LogFunction : ParserFunction, INumericFunction
    {
        protected override Variable Evaluate(ParsingScript script)
        {
            ...
            Utils.CheckArgs(args.Count, 1, m_name, true);
            Variable arg = args[0];
            arg.Value = System.Math.Log(arg.Value);
            return arg;
        }
    }
